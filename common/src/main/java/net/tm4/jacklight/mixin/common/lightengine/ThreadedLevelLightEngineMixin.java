...
public abstract class ThreadedLevelLightEngineMixin extends LevelLightEngine implements JackLightLightingProvider {
    ...
    private void queueTaskForSection(final int chunkX, final int chunkY, final int chunkZ,
                                     final Supplier<JackLightInterface.LightQueue.ChunkTasks> runnable) {
        ...
        final JackLightInterface.LightQueue.ChunkTasks updateFuture = runnable.get();
        ...
        if (references == 0) {
            ...
            world.getChunkSource().addRegionTicket(JackLightInterface.CHUNK_WORK_TICKET, pos, 0, pos);
        }
        ...
        updateFuture.onComplete.thenAcceptAsync((final Void ignore) -> {
            ...
            if (newReferences == 1) {
                ...
                final ChunkPos pos = new ChunkPos(chunkX, chunkZ);
                world.getChunkSource().removeRegionTicket(JackLightInterface.CHUNK_WORK_TICKET, pos, 0, pos);
            } else {
    ...
    public CompletableFuture<ChunkAccess> lightChunk(final ChunkAccess chunk, final boolean lit) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            final Boolean[] emptySections = JackLightEngine.getEmptySectionsForChunk(chunk);
            if (!lit) {
